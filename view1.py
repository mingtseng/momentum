...
class MainPage(tk.Frame):
    ...
    def get_data(self, key):
        # 获取项目内tfl的数量, 因为每页的tracker最多容纳100个tfl, 所以用总数量除以100再向上取整就获得了页码数
        url_page = "http://10.20.2.73:8082/system/tfls/ajax_list?study_number=" + self.ProjectName.get() + "-" + self.FolderName.get() + "&page=1&limit=15"
        response_page = self.re_session.get(url_page, headers=dict(referer=url_page)).text
        pagenumber = json.loads(response_page)['count']
        items = []
        for page in range(1, math.ceil(pagenumber / 100) + 1):
            url_goal_page = "http://10.20.2.73:8082/system/tfls/ajax_list?study_number=" + self.ProjectName.get() + "-" + self.FolderName.get() + "&page=" + str(page) + "&limit=100"
            response_page = self.re_session.get(url_goal_page, headers=dict(referer=url_goal_page)).text
            items = items + json.loads(response_page)['data']
    ...
    def get_adam_bat(self):
        if tk.messagebox.askokcancel(title="tips", message="Proceed?"):
    ...
    def get_adam(self):
        ...
        if tk.messagebox.askokcancel(title="tips", message="Proceed?"):
            ...
            messagebox.showinfo('tips', 'ADaM generated successfully')
    ...
    def get_tfl(self):
        ...
        if tk.messagebox.askokcancel(title="tips", message="Proceed?"):
            ...
            for a, b, t in zip(self.get_data(key='source_program_name'), self.get_data(key='title'), self.get_data(key="file_type")):
